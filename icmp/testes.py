...
dataset = pd.read_csv('data/datasetsallFinal.csv') # read & prepare data
...
trainScalern, testScalern, getYtrain, getYtest = train_test_split(X, Yb, test_size = 0.5)
dropTrain = pd.DataFrame(trainScalern, columns=columnz) # call 'X test' array and make them to dataframe
dropTrain.to_csv(r'dataSample/encode/scaler/train_scaler.csv') # save to csv

dropTest = pd.DataFrame(testScalern, columns=columnz) # call 'X test' array and make them to dataframe
dropTest.to_csv(r'dataSample/encode/scaler/test_scaler.csv') # save to csv

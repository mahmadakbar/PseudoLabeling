...
# 90:10 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
train90d, test10d = train_test_split(datasetsAll, test_size=0.1)
train90d = train90d.reset_index(drop=True)
test10d = test10d.reset_index(drop=True)

train90d.to_csv(r'dataSample/train90d.csv')
test10d.to_csv(r'dataSample/test10d.csv')

# train
train90d['source_ip'] = train90d.source_ip.apply(ip2int)
train90d['timestamp'] = train90d.timestamp.apply(iso2times)

# test
test10d['source_ip'] = test10d.source_ip.apply(ip2int)
test10d['timestamp'] = test10d.timestamp.apply(iso2times)

encodeTrain90d = MultiColumnLabelEncoder(columns = transfoorm).fit_transform(train90d)
encodeTest10den = MultiColumnLabelEncoder(columns = transfoorm).fit_transform(test10d)
getType10d = encodeTest10den['type'].values
encodeTest10d = encodeTest10den.drop(['type'], axis =1)

encodeTrain90d.to_csv(r'dataSample/encode/train90d.csv')
encodeTest10d.to_csv(r'dataSample/encode/test10d.csv')

# Prepare for execution
Y_data90d = encodeTrain90d['type'].values
X_data90d = encodeTrain90d[list(columns)].values
X_test10d = encodeTest10d[list(columns)].values
...
pseudoY_test10e10 = lin_clf10e10.predict(encodeTest10d)
...
dfsubmit10e10 = pd.concat([encodeTest10d['_id'], dfprediction10e10['type']], axis = 1, join_axes=[encodeTest10d['_id'].index])
...
pseudoY_test10e50 = lin_clf10e50.predict(encodeTest10d)
...
dfsubmit10e50 = pd.concat([encodeTest10d['_id'], dfprediction10e50['type']], axis = 1, join_axes=[encodeTest10d['_id'].index])
...
pseudoY_test10e100 = lin_clf10e100.predict(encodeTest10d)
...
dfsubmit10e100 = pd.concat([encodeTest10d['_id'], dfprediction10e100['type']], axis = 1, join_axes=[encodeTest10d['_id'].index])
...
print("\n")
# 80:20 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
train80d, test20d = train_test_split(datasetsAll, test_size=0.2)
train80d = train80d.reset_index(drop=True)
test20d = test20d.reset_index(drop=True)

train80d.to_csv(r'dataSample/train80d.csv')
test20d.to_csv(r'dataSample/test20d.csv')

# train
train80d['source_ip'] = train80d.source_ip.apply(ip2int)
train80d['timestamp'] = train80d.timestamp.apply(iso2times)

# test
test20d['source_ip'] = test20d.source_ip.apply(ip2int)
test20d['timestamp'] = test20d.timestamp.apply(iso2times)

encodeTrain80d = MultiColumnLabelEncoder(columns = transfoorm).fit_transform(train90d)
encodeTest20den = MultiColumnLabelEncoder(columns = transfoorm).fit_transform(test10d)

getType20d = encodeTest20den['type'].values
encodeTest20d = encodeTest20den.drop(['type'], axis =1)

encodeTrain80d.to_csv(r'dataSample/encode/train80d.csv')
encodeTest20d.to_csv(r'dataSample/encode/test20d.csv')

# Prepare for execution
Y_data80d = encodeTrain80d['type'].values
X_data80d = encodeTrain80d[list(columns)].values
X_test20d = encodeTest20d[list(columns)].values
...
pseudoY_test20e10 = lin_clf20e10.predict(encodeTest20d)
...
dfsubmit20e10 = pd.concat([encodeTest20d['_id'], dfprediction20e10['type']], axis = 1, join_axes=[encodeTest20d['_id'].index])
...
pseudoY_test20e50 = lin_clf20e50.predict(encodeTest20d)
...
dfsubmit20e50 = pd.concat([encodeTest20d['_id'], dfprediction20e50['type']], axis = 1, join_axes=[encodeTest20d['_id'].index])
...
pseudoY_test20e100 = lin_clf20e100.predict(encodeTest20d)
...
dfsubmit20e100 = pd.concat([encodeTest20d['_id'], dfprediction20e100['type']], axis = 1, join_axes=[encodeTest20d['_id'].index])
...
print("\n")
# 70:30 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
train70d, test30d = train_test_split(datasetsAll, test_size=0.3)
train70d = train70d.reset_index(drop=True)
test30d = test30d.reset_index(drop=True)

train70d.to_csv(r'dataSample/train70d.csv')
test30d.to_csv(r'dataSample/test30d.csv')

# train
train70d['source_ip'] = train70d.source_ip.apply(ip2int)
train70d['timestamp'] = train70d.timestamp.apply(iso2times)

# test
test30d['source_ip'] = test30d.source_ip.apply(ip2int)
test30d['timestamp'] = test30d.timestamp.apply(iso2times)

encodeTrain70d = MultiColumnLabelEncoder(columns = transfoorm).fit_transform(train90d)
encodeTest30den = MultiColumnLabelEncoder(columns = transfoorm).fit_transform(test10d)

getType30d = encodeTest30den['type'].values
encodeTest30d = encodeTest30den.drop(['type'], axis =1)

encodeTrain70d.to_csv(r'dataSample/encode/train70d.csv')
encodeTest30d.to_csv(r'dataSample/encode/test30d.csv')

# Prepare for execution
Y_data70d = encodeTrain70d['type'].values
X_data70d = encodeTrain70d[list(columns)].values
X_test30d = encodeTest30d[list(columns)].values
...
pseudoY_test30e10 = lin_clf30e10.predict(encodeTest30d)
...
dfsubmit30e10 = pd.concat([encodeTest30d['_id'], dfprediction30e10['type']], axis = 1, join_axes=[encodeTest30d['_id'].index])
...
pseudoY_test30e50 = lin_clf30e50.predict(encodeTest30d)
...
dfsubmit30e50 = pd.concat([encodeTest30d['_id'], dfprediction30e50['type']], axis = 1, join_axes=[encodeTest30d['_id'].index])
...
pseudoY_test30e100 = lin_clf30e100.predict(encodeTest30d)
...
dfsubmit30e100 = pd.concat([encodeTest30d['_id'], dfprediction30e100['type']], axis = 1, join_axes=[encodeTest30d['_id'].index])
...
print("\n")
# 60:40 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
train60d, test40d = train_test_split(datasetsAll, test_size=0.4)
train60d = train60d.reset_index(drop=True)
test40d = test40d.reset_index(drop=True)

train60d.to_csv(r'dataSample/train60d.csv')
test40d.to_csv(r'dataSample/test40d.csv')

# train
train60d['source_ip'] = train60d.source_ip.apply(ip2int)
train60d['timestamp'] = train60d.timestamp.apply(iso2times)

# test
test40d['source_ip'] = test40d.source_ip.apply(ip2int)
test40d['timestamp'] = test40d.timestamp.apply(iso2times)

encodeTrain60d = MultiColumnLabelEncoder(columns = transfoorm).fit_transform(train90d)
encodeTest40den = MultiColumnLabelEncoder(columns = transfoorm).fit_transform(test10d)

getType40d = encodeTest40den['type'].values
encodeTest40d = encodeTest40den.drop(['type'], axis =1)

encodeTrain60d.to_csv(r'dataSample/encode/train60d.csv')
encodeTest40d.to_csv(r'dataSample/encode/test40d.csv')

# Prepare for execution
Y_data60d = encodeTrain60d['type'].values
X_data60d = encodeTrain60d[list(columns)].values
X_test40d = encodeTest40d[list(columns)].values
...
pseudoY_test40e10 = lin_clf40e10.predict(encodeTest40d)
...
dfsubmit40e10 = pd.concat([encodeTest40d['_id'], dfprediction40e10['type']], axis = 1, join_axes=[encodeTest40d['_id'].index])
...
pseudoY_test40e50 = lin_clf40e50.predict(encodeTest40d)
...
dfsubmit40e50 = pd.concat([encodeTest40d['_id'], dfprediction40e50['type']], axis = 1, join_axes=[encodeTest40d['_id'].index])
...
pseudoY_test40e100 = lin_clf40e100.predict(encodeTest40d)
...
dfsubmit40e100 = pd.concat([encodeTest40d['_id'], dfprediction40e100['type']], axis = 1, join_axes=[encodeTest40d['_id'].index])
...
print("\n")
# 50:50 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
train50d, test50d = train_test_split(datasetsAll, test_size=0.5)
train50d = train50d.reset_index(drop=True)
test50d = test50d.reset_index(drop=True)

train50d.to_csv(r'dataSample/train50d.csv')
test50d.to_csv(r'dataSample/test50d.csv')

# train
train50d['source_ip'] = train50d.source_ip.apply(ip2int)
train50d['timestamp'] = train50d.timestamp.apply(iso2times)

# test
test50d['source_ip'] = test50d.source_ip.apply(ip2int)
test50d['timestamp'] = test50d.timestamp.apply(iso2times)

encodeTrain50d = MultiColumnLabelEncoder(columns = transfoorm).fit_transform(train90d)
encodeTest50den = MultiColumnLabelEncoder(columns = transfoorm).fit_transform(test10d)

getType50d = encodeTest50den['type'].values
encodeTest50d = encodeTest50den.drop(['type'], axis =1)

encodeTrain50d.to_csv(r'dataSample/encode/train50d.csv')
encodeTest50d.to_csv(r'dataSample/encode/test50d.csv')

# Prepare for execution
Y_data50d = encodeTrain50d['type'].values
X_data50d = encodeTrain50d[list(columns)].values
X_test50d = encodeTest50d[list(columns)].values
...
pseudoY_test50e10 = lin_clf50e10.predict(encodeTest50d)
...
dfsubmit50e10 = pd.concat([encodeTest50d['_id'], dfprediction50e10['type']], axis = 1, join_axes=[encodeTest50d['_id'].index])
...
pseudoY_test50e50 = lin_clf50e50.predict(encodeTest50d)
...
dfsubmit50e50 = pd.concat([encodeTest50d['_id'], dfprediction50e50['type']], axis = 1, join_axes=[encodeTest50d['_id'].index])
...
pseudoY_test50e100 = lin_clf50e100.predict(encodeTest50d)
...
dfsubmit50e100 = pd.concat([encodeTest50d['_id'], dfprediction50e10['type']], axis = 1, join_axes=[encodeTest50d['_id'].index])
